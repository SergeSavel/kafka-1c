...
Так обращения из 1С не выходят за пределы машины: это быстрее, не нагружает сеть и не требует защищать канал до шлюза. В настройках [кластера](cluster.md) при этом достаточно оставить адрес `localhost` — каждый сеанс обмена обратится к шлюзу своего сервера.
