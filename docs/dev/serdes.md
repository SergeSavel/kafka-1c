...
Методы [`СообщениеКлюч` и `СообщениеЗначение`](handler.md) обработки обмена вызываются на каждый выгружаемый объект. Сердес `Игнорировать` для выгрузки запрещён — это проверяется при записи шины; `Null` допускается и означает обмен только надгробиями: обработчик обязан возвращать `Null`.
