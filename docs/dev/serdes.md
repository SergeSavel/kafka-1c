...
## Отправка: что должна вернуть обработка

Методы [`СообщениеКлюч` и `СообщениеЗначение`](handler.md) обработки обмена вызываются на каждый выгружаемый объект. Сердес `Игнорировать` для выгрузки запрещён — это проверяется при записи шины; `Null` допускается и означает обмен только надгробиями: обработка обязана возвращать `Null`.
...
| Что вернула обработка | Результат |
| --- | --- |
