...
!!! note "Прикладной код обычно не создаёт адаптер вручную"
    В [обработке обмена](handler.md) готовый адаптер уже передаётся в события `ПередВыгрузкойШины`, `ПередВыгрузкойПакета` и `ПередЗагрузкойРаздела` — там достаточно воспользоваться им, не создавая собственный.
