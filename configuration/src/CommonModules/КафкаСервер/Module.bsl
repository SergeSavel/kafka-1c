...
Процедура ЗагрузитьРазделы_(ШинаИнфо, Обработка, Адаптер, Consumer, Partitions, МоментЗавершения, РодительскийСеансИнфо)
	...
	СостояниеЧтения = ПолучитьСостояниеЧтения(ШинаИнфо.Кластер, Адаптер, Consumer, Partitions);
...
Функция ПолучитьСостояниеЧтения(Кластер, Адаптер, Consumer, Partitions, ПолучатьНачальное = Истина, ПолучатьКонечное = Истина, ПолучатьТекущее = Истина)
...
Функция КонфигурацияОтправителя(Кластер) Экспорт
...
Функция КонфигурацияПолучателя(Кластер) Экспорт
...
Функция КластерИнфо(Кластер) Экспорт
...
Функция ПолучитьЧитаемыеРазделы(Адаптер, Consumer, Тема)
	...
	ЭтоШаблон = Ложь;
	ЭтоШаблон = ЭтоШаблон Или (СтрНайти(Тема, "*") > 0);
	ЭтоШаблон = ЭтоШаблон Или (СтрНайти(Тема, "?") > 0);
	ЭтоШаблон = ЭтоШаблон Или (СтрНайти(Тема, "+") > 0);
	...
	Если ЭтоШаблон Тогда
		...
		TopicInfos = Адаптер.ConsumerListTopics(Consumer, Шаблон);
	...
	Иначе
		...
		TopicInfo = Адаптер.ConsumerListPartitions(Consumer, Тема);
		Если TopicInfo = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьГраничныеСмещенияТемы(Кластер, Адаптер, Consumer, Тема)
	...
	Результат = ПолучитьСостояниеЧтения(Кластер, Адаптер, Consumer, Partitions, Истина, Истина, Ложь);
