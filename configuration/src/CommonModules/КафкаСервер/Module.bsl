...
Процедура ЗагрузитьРазделы_(ШинаИнфо, Обработка, Адаптер, Consumer, Partitions, МоментЗавершения, РодительскийСеансИнфо)
	...
	Если ОсталосьРазделов > 0 Тогда
		...
		ИмяПроцедурыЗагрузки = "Обработка." + ШинаИнфо.ПриемПроцедура + "(ТекущееСообщение)";
		...
		Пока ОсталосьРазделов > 0 Цикл
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				СостояниеЧтенияРаздела = СостояниеЧтения[Сообщение.Тема][Сообщение.Раздел];
				...
				Если ТекущийМомент - МоментПоследнейФиксации >= 20000 Тогда
...
Функция УдалитьПолучитьСостояниеЧтения(Кластер, Адаптер, Consumer, Partitions, ПолучатьНачальное = Истина, ПолучатьКонечное = Истина, ПолучатьТекущее = Истина)
	...
	Если ПолучатьТекущее Тогда
		
		Для Каждого КлючЗначениеТема Из СостояниеЧтения Цикл
			Для Каждого КлючЗначениеРаздел Из КлючЗначениеТема.Значение Цикл
				СостояниеЧтенияРаздела = КлючЗначениеРаздел.Значение;
				ТекущееСмещение = РегистрыСведений.КафкаПозиции.ПолучитьПозицию(Кластер, КлючЗначениеТема.Ключ, КлючЗначениеРаздел.Ключ);
...
Функция ПолучитьСообщения(ШинаИнфо, Адаптер, Consumer)
	...
	Если Версия = 1 Тогда
		...
		Для НомерСообщения = 1 По КоличествоСообщений Цикл
			...
			Результат.Добавить(Новый ФиксированнаяСтруктура(Сообщение));
