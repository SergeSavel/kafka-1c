...
Процедура ВыгрузитьПакетОтдельныйСеанс(ШинаИнфо, КластерИнфо, Producer, Пакет, РодительскийСеансИнфо) Экспорт
	...
	КомпьютерИмя = ИмяКомпьютера();
	ШлюзАдрес = КластерИнфо.ШлюзАдрес;
	Если СтрСравнить(КомпьютерИмя, РодительскийСеансИнфо.СерверИмя) <> 0 Тогда
		Если ШлюзАдрес = Неопределено Или ПустаяСтрока(ШлюзАдрес) Тогда
			ШлюзАдрес = КомпьютерИмя;
		ИначеЕсли СтрСравнить(ШлюзАдрес, "localhost") = 0 Тогда
			ШлюзАдрес = КомпьютерИмя;
		ИначеЕсли СтрСравнить(Лев(ШлюзАдрес, 10), "localhost:") = 0 Тогда
			ШлюзАдрес = КомпьютерИмя + Сред(ШлюзАдрес, 10);
		КонецЕсли;		
	КонецЕсли;
