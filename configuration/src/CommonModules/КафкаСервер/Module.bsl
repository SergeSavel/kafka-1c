...
Процедура Выгрузить(Шина) Экспорт
	...
	Адаптер.Инициализировать(
			КластерИнфо.ПроксиАдрес,
			КластерИнфо.ПроксиПользователь,
			КластерИнфо.ПроксиПароль,
			КластерИнфо.ПроксиТаймаут
	);
...
Процедура ВыгрузитьПакетОтдельныйСеанс(ШинаИнфо, КластерИнфо, Producer, Пакет, РодительскийСеансИнфо) Экспорт
	...
	// используем прокси на сервере родительского сеанса.
	КомпьютерИмя = ИмяКомпьютера();
	ПроксиАдрес = КластерИнфо.ПроксиАдрес;
	Если СтрСравнить(КомпьютерИмя, РодительскийСеансИнфо.СерверИмя) <> 0 Тогда
		Если ПроксиАдрес = Неопределено Или ПустаяСтрока(ПроксиАдрес) Тогда
			ПроксиАдрес = КомпьютерИмя;
		ИначеЕсли СтрСравнить(ПроксиАдрес, "localhost") = 0 Тогда
			ПроксиАдрес = КомпьютерИмя;
		ИначеЕсли СтрСравнить(Лев(ПроксиАдрес, 10), "localhost:") = 0 Тогда
			ПроксиАдрес = КомпьютерИмя + Сред(ПроксиАдрес, 10);
		КонецЕсли;		
	КонецЕсли;
