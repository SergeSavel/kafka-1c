...
Функция ВыгрузитьОбъект(ШинаИнфо, Обработка, Producer, Адаптер, Объект, ОбъектМетаданныхИнфо, Заголовки)
	...
	Попытка
		...
		// Раздел
		// За назначение раздела отвечает экземпляр отправителя внутри шлюза.
		Раздел = Неопределено;
	...
	Попытка
		...
		Если Версия = Неопределено Тогда
			ВызватьИсключение "Неожиданный конец потока.";
		КонецЕсли;
		...
		Если Версия = 1 Тогда
			...
			Если SerializedKeySize = Неопределено Тогда
				ВызватьИсключение "Неожиданный конец потока.";
			КонецЕсли;
			
			SerializedValueSize = ЧтениеДанных.ПрочитатьЦелое32();
			Если SerializedValueSize = Неопределено Тогда
				ВызватьИсключение "Неожиданный конец потока.";
			КонецЕсли;
